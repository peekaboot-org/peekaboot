...
actually has to be drawn, so no other page loads it, and neither does a dashboard that
never opens the tab.
...
measures 13.2:1 on white and 1.4:1 on the dark theme's `--pk-bg`, where it disappears.
`logo-mark-dark.png` is the same artwork with that slate recoloured to a light neutral.
Both surfaces swap it with a CSS `background-image` override rather than swapping an
`<img src>`, so no JavaScript is involved:
...
issue". That is the backend's per-trace `slow` flag, set by `IssueDetector`. It is
deliberately not "this trace is in the Slow bucket". The bucket admits a trace by its total
duration (`slowTraceThresholdMs`, 1 s by default); the badge reports a single span at or
above the span threshold (100 ms by default). A 300 ms request with one 150 ms query shows
the badge under *All* and never appears under *Slow*; a 1.2 s request made of twelve 100 ms
spans appears under *Slow* and shows the badge too.
...
surface's own sheet as `<link>` elements inside the shadow root. A linked sheet in a shadow
root loads asynchronously, so `attachSharedStyles` holds `hostElement.style.visibility = 'hidden'`
until every sheet has settled (`load` or `error`), then reveals it. That is the FOUC
...
- `attachSharedStyles` must be called exactly once per shadow root. It doesn't guard
  ...
  replacing the first. Its one remaining call site makes this safe by construction, since
  `trace-detail.js`'s `openTraceDetail()` always calls `closeTraceDetail()` (which
  removes the whole host element) before creating a new host and shadow root. A future
  call site needs the same discipline.
...
not own. It is linked as a classic, non-deferred script rather than written inline. A
module would run after first paint, and an inline block is dropped by a host whose CSP
...
their shadow host element, not the shadow root, which has no attributes. That is exactly
why the dark-mode selector in `tokens.css`/`base.css` is written
...
- **A saturated fill (`--pk-success`, `--pk-warning`, `--pk-danger`, `--pk-primary`,
  `--pk-info`, `--pk-purple`) needs its own on-colour token as foreground:**
  `--pk-on-success`, `--pk-on-warning`, `--pk-on-danger`, `--pk-on-primary`,
  `--pk-on-info`, `--pk-on-purple` respectively.
  **Never `--pk-text-strong` or literal `white`/`#fff` on one of these fills.**
  ...
  `--pk-on-*` is dark ink in both themes, since white on the green fill is 2.61:1. The
  exceptions are `--pk-on-purple`, `--pk-on-info` and `--pk-on-warning`, which are white
  ink in light mode, because their fills sit in the deep tier beside `--pk-danger`
  (5.70:1, 5.36:1 and 4.83:1 under white).
...
- **Interactive elements are real controls with `:focus-visible`**, not `div`/`role`
  ...
  chrome reset away, not `<span>`s with click handlers, so each is reachable by keyboard:
  the gantt expand/collapse triangle, the SQL and logs toggles, the gantt event markers,
  the "show logs for all spans" link, the log span-filter cell and the span-filter clear.
  If a new control needs `cursor: pointer`, that is the smell; make it a `<button>` first.
...
- **Motion respects `prefers-reduced-motion`.** `base.css` cancels animations and
  ...
  long as the surface is open. Nothing animated is load-bearing, since the spinner is
  always paired with "Loading data..." text, so cancelling motion never removes meaning.
...
continuation or a streamed body, still reaches the bar. The last attempt lands at 4.75s
...
1. Create `dashboard/tabs/<id>.js` exporting:
   ...
   - optionally `export function isAvailable(data, features) { ... }`, which gates whether
     the tab's strip button is shown at all (see `meters.js`/`traces.js` for real examples
     gating on a feature flag).
   - optionally `export function applyFilter(payload, context) { ... }`, which lets another
