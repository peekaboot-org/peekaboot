...
public class OtelSpanExporter implements SpanExporter {
    ...
    private net.osslabz.peekaboot.backend.tracing.store.SpanData convertToSpanData(SpanData otelSpan) {
        ...
        List<net.osslabz.peekaboot.backend.tracing.store.SpanData.Event> events = extractEvents(otelSpan);
        ...
        return new net.osslabz.peekaboot.backend.tracing.store.SpanData(
    ...
    private List<net.osslabz.peekaboot.backend.tracing.store.SpanData.Event> extractEvents(SpanData otelSpan) {
        return otelSpan.getEvents().stream()
                .map(this::convertEvent)
                .toList();
    }

    private net.osslabz.peekaboot.backend.tracing.store.SpanData.Event convertEvent(EventData event) {
        return new net.osslabz.peekaboot.backend.tracing.store.SpanData.Event(
                event.getName(),
