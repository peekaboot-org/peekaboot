...
class NestedDispatchTraceCaptureIT {
    ...
    void forwardingRequestCapturesBothDispatches() {
        JsonNode trace = traces.awaitTrace(traces.triggerAndCaptureTraceId("/people"));
    ...
    void forwardingRequestDoesNotAbsorbTheNextRequestOnItsThread() {
        String forwardTraceId = traces.triggerAndCaptureTraceId("/people");
        ...
        for (int i = 0; i < 5; i++) {
            followUpTraceIds.add(traces.triggerAndCaptureTraceId("/persons"));
        }
        ...
        JsonNode trace = traces.awaitTrace(forwardTraceId);
