 ...
 * ({@code ToolbarTest.toolbarShowsErrorLogCountWhenRequestLogsAnError} and
 * {@code TraceOverlayTest.logsFilterChipUsesTheContrastTunedForeground}) fail with an
...
@ActiveProfiles("test")
class LogCaptureIntegrationTest {
    ...
    void errorLoggedInsideRequestIsCapturedAgainstThatRequestsTrace() {
        ...
        assertThat(logs.isMissingNode() || logs.isNull())
                .as("trace %s carried spans but no logs section at all - nothing reached "
                  + "PeekabootLogbackAppender, so MDC correlation is not populating traceId", traceId)
                .isFalse();

        assertThat(logs.path("errorCount").asInt())
                .as("the ERROR PersonController logs for /?error=true must be captured against "
                  + "trace %s; a count of 0 means the log event carried no MDC traceId", traceId)
                .isEqualTo(1);
    }
    ...
    private String requestTraceIdFor(String path) {
        ...
        assertThat(matcher.find())
                .as("dev toolbar must embed a trace id for %s - without one the request was "
                  + "never traced and the log-capture assertion would be meaningless", path)
                .isTrue();
    ...
    private JsonNode awaitExportedTrace(String traceId) {
        ...
        throw new AssertionError("trace " + traceId + " never had an exported span within "
                + TRACE_EXPORT_TIMEOUT + "; last response: " + lastSeen);
    }
