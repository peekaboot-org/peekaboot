...
The snapshot is a cache, never a source of truth. Anything wrong with it costs only the
history: a bad magic number, a schema version this build doesn't know, a ring geometry
that no longer matches `peekaboot.insights.levels`, or an age past
`peekaboot.insights.persistence.max-age` (default: the coarsest level's span). The file
is deleted and the rings start empty, exactly as they would with storage off. Every
length read from the file is checked against a plausibility bound before it is used to
allocate, so a corrupt file can never provoke an oversized allocation.
...
`OwnerOnlyFiles.replaceAtomically` on every change. That is cheap at this size (≤400 KB
for the full 1000), and it removes both a partial-line corruption window and a second
trim code path. A line that fails to parse
...
memory and serves them from there; `LifecycleEventFile` is never consulted. This
...
log is itself a start, with no stop in between, is flagged `uncleanPrevious`. Computing
...
7. **Thresholds**: `IssueDetector` raises SLOW/VERY_SLOW/SLOW_QUERY at `UiTracingProperties`' thresholds and sets each
   `TraceTree.slow` (some span carries SLOW or VERY_SLOW, which is the Traces tab's badge); `GET /peekaboot/api/features`
...
writes into a page (script, stylesheets, dashboard links, the `basePath` in its data
blob) is prefixed with `request.getContextPath()`. On the browser side `shared/api.js`
...
`/`, management endpoints at the application root, excludes nothing extra, since there is
no prefix to tell them apart by). The bean also carries the resolved
...
populated by `RequestCaptureFilter`. They are not captured, regardless of dev-toolbar. Those
...
`InsightsAutoConfiguration` hands the same mapper to `InsightsSsePublisher`. The
...
design system: the standalone dashboard, the dev toolbar injected into host-app pages,
and the trace-detail overlay. See `peekaboot-frontend/README.md` for the full
...
Auto-configuration classes that wire everything together. The ten `@AutoConfiguration`
...
| `PeekabootDefaultsEnvironmentPostProcessor` | `spring.factories` (`EnvironmentPostProcessor`) | Local-dev detection for `peekaboot.enabled`, `peekaboot.dev-toolbar` and `peekaboot.storage.enabled` + default property values |
...
Auto-configuration uses Spring Boot conditionals. Eight of the ten classes carry the
same two class-level conditions, the servlet guard and the master switch:
`PeekabootAutoConfiguration`, `ActuatorSourcesAutoConfiguration`,
...
need the `HealthEndpoint` and `InfoEndpoint` classes, `DevToolbarAutoConfiguration`
additionally requires `peekaboot.dev-toolbar`, `TracingInterceptorAutoConfiguration` an
`ObservationRegistry` bean, `InsightsAutoConfiguration` a `MeterRegistry` bean and
`peekaboot.insights.enabled`, `OtelTracingAutoConfiguration` the OpenTelemetry SDK's
`SpanExporter` class.

`PeekabootAutoConfiguration` registers the servlet-only `PeekabootWebConfig` next to the
controllers, services and actuator wiring, all as explicit `@Bean` methods, each
`@ConditionalOnMissingBean` so an application bean of the same type or name replaces the
default instead of colliding with it (the method names yield the class-derived bean names
`ServerUrlResolver`'s dashboard check relies on). It needs the servlet guard because
...
mappers, just with nothing servlet-specific ever invoking them. That is dead beans and
...
class-level servlet guard, because the ready/stopped summaries, the run history and the
storage directory must work in a plain non-web application. Only the lifecycle
API's `LifecycleController` bean is servlet-gated on its own. `PeekabootDefaultsEnvironmentPostProcessor`
applies the same split to the property defaults: the activation and storage detection is
web-type independent, while `peekaboot-defaults.yml` and the dev-toolbar defaults are
skipped for a non-servlet application (see *Default Properties*).
...
Its capture half, the `RequestCaptureFilter` registration and the Logback
appender registrar, is `@ConditionalOnBean(TraceStore.class)` instead. With
...
which adds them to a `peekabootDetection` property source at lowest precedence, so any
...
explicitly in a shared environment does not get the toolbar injected into every page as
a side effect.
...
4. Those three hold for *every* exploded-classpath launch, so two more signals decide
   ...
   `out/production/`. An IDE, `spring-boot:run` and `bootRun` always put one
   there; a Jib image (`/app/classes`) or Boot's `extract` layout (a thin jar with a
...
- `peekaboot-defaults.yml`. Enables full observability, but only when Peekaboot is
  enabled *and* the application is a servlet web application. It is skipped entirely
  otherwise, since everything that would read it (the dashboard, the filters, the trace
  store) is servlet-only. The web type is `spring.main.web-application-type` where the
  environment carries it, and otherwise the one `SpringApplication.getWebApplicationType()`
  deduced, because Boot binds `spring.main.*` onto the application only after the
  post-processors have run. The dev-toolbar defaults below sit behind the same check.
- `peekaboot-no-push-defaults.yml`. Applies unconditionally, even when Peekaboot
  itself is disabled, to keep telemetry from leaving the process by default.
- `peekaboot-dev-toolbar-defaults.yml`. Applied only when the dev toolbar resolves
...
setting. Where it does not, Peekaboot's four named sources are appended last, in that
order: `peekabootDetection`, `peekabootNoPushDefaults`, `peekabootDefaults` and
...
- Is one more `SpanExporter` bean alongside whatever Spring Boot's own OpenTelemetry
  ...
  (sampling, other exporters such as Zipkin, Jaeger or an OTLP backend) untouched.
...
- Records an error only for a span whose status code is `ERROR`. The message is the status
  ...
  description is empty, and the class is that event's `exception.type`, or `ERROR` where the
...
in between is traced with no logs against it. One application never sees this, since its
...
`jdbc.datasource.name` are ignored. That is the shape produced when a single operation is
...
trace is flagged `truncated: true`. That flag is exposed on both
`GET /peekaboot/api/traces/insights` and `GET /peekaboot/api/traces/{traceId}/insights`,
and shown as a `TRUNCATED` badge in the trace list and the trace-detail overlay.
...
it within reach. The answer to that is a few lines in `TraceDataBundle`, never a second
...
span: the CLIENT side of a database call carrying a `db.*` or `jdbc.query*` tag. `jdbc.*`
alone is not enough, since datasource-proxy's connection and result-set spans carry
`jdbc.datasource.name`/`jdbc.row-count` and are not queries. It is shared by
...
its socket, since the heartbeat and the lane overflow already detect one that is merely
wedged. Every expiry costs a full resync, which for the default 39 series is roughly
393,000 values per open dashboard at level 1. Thirty minutes would cut that by an order of
magnitude with nothing functional lost; five is the value it was built with, nothing more.
...
(`CpuTopology`; both only where cheaply readable, from `/proc/cpuinfo` on Linux, with no
forking), total physical memory, the JVM's max heap, the machine's non-local IP addresses
with their reverse-resolved hostnames (`NetworkAddress`, up interfaces only, loopback and
...
and no embedded server. A bare `AnnotationConfigApplicationContext` is fine where a
bean-name lookup needs a real container (`ServerUrlResolverTest`).
...
module here, and `spring-boot-micrometer-metrics` for the
`CompositeMeterRegistryAutoConfiguration` edge in `InsightsAutoConfiguration`. Boot reads
ordering edges from the class metadata without loading the named classes, which is why
`TracingInterceptorAutoConfiguration` can use a class literal
(`after = ObservationAutoConfiguration.class`) for a module the pom marks `<optional>`.
The class only has to exist at compile time.
