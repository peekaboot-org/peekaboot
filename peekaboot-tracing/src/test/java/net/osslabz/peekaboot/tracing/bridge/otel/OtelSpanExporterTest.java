...
class OtelSpanExporterTest {
    ...
    void shouldConvertAndStoreOtelSpan() {
        ...
        List<net.osslabz.peekaboot.tracing.store.SpanData> spans = store.getSpansForTrace(traceId);
        assertThat(spans).hasSize(1);

        net.osslabz.peekaboot.tracing.store.SpanData stored = spans.getFirst();
    ...
    void shouldMapAllSpanKinds() {
        ...
        for (SpanKind kind : SpanKind.values()) {
            ...
            List<net.osslabz.peekaboot.tracing.store.SpanData> stored = store.getSpansForTrace(traceId);
